...
%{
	...
	if ~exist('probeset_signif_arr','var')
		probeset_signif_arr = [];
		for i=1:k_sz
			if mod(i,1000) == 0
				fprintf('Iteration %d\n', i);
			...
			a_arr = current_row(1:3);
			b_arr = current_row(4:6);
			e_arr = current_row(13:15);
			% ab_p = ranksum(a_arr, b_arr);
			% ae_p = ranksum(a_arr, e_arr);
			% be_p = ranksum(b_arr, e_arr);
			% probeset_signif_arr = [probeset_signif_arr; [ab_p ae_p be_p]];
			[h,ab_p] = ttest(a_arr, b_arr);
			[h,ae_p] = ttest(a_arr, e_arr);
			[h,be_p] = ttest(b_arr, e_arr);
			probeset_signif_arr = [probeset_signif_arr; [ab_p ae_p be_p]];
		end
	end
	...
	dn_rows = {};
	for i=1:k_sz
		% AB are significantly differe, BE are significantly different, AE are not significantly different
		if probeset_signif_arr(i,1) <= alpha && probeset_signif_arr(i,3) <= alpha && probeset_signif_arr(i,2) > alpha
			% fprintf('%.4f   -   %.4f\t%.4f\t%.4f\n', alpha, probeset_signif_arr(i,1), probeset_signif_arr(i,3), probeset_signif_arr(i,2));
			current_row = mapObj(k{i});
			mu_a = mean(current_row(1:3));
			mu_b = mean(current_row(4:6));
			mu_e = mean(current_row(13:15));
			if mu_a < mu_b && mu_e < mu_b
				dn_rows = [dn_rows k{i}];
			elseif mu_a > mu_b && mu_e > mu_b
				up_rows = [up_rows k{i}];
			end
		end
	end
	fprintf('Found %d probesets significantly LOWER in A and E from B\n', numel(dn_rows));
	fprintf('Found %d probesets significantly HIGHER in A and E from B\n', numel(up_rows));

	fid = fopen('output/sig_genes_ttest.txt', 'w');
	fprintf(fid, 'Direction of Change B to AE\tGene Name\tParent Groups\tParent Extended Groups\tProbeset ID\tA1\tA2\tA3\tB1\tB2\tB3\tE1\tE2\tE3\tPval AB\tPval BE\tPval AE\n');

	fprintf('Recording DOWN rows...\n');
	dn_save_arr = print_significant_genes(dn_rows, 0, genes_orig, genes_new, mapObj, fid, db_obj, false);
	fprintf('Recording UP rows...\n');
	up_save_arr = print_significant_genes(up_rows, 1, genes_orig, genes_new, mapObj, fid, db_obj, false);
	fclose(fid);
%}
else
	% heatmaps
	...
	for i=3:3
		...
		for j=2:numel(genes_orig{i})
			...
			for k=1:numel(res)
				if ismember(res(k).probeset, significant_probesets)
